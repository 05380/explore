...
void SDFMap::initMap(ros::NodeHandle& nh) {
  ...
  nh.param("partitioning/use_swarm_tf", mp_->use_swarm_tf_, false);
...
void SDFMap::inputPointCloud(const pcl::PointCloud<pcl::PointXYZ>& points, const int& point_num,
  ...
  while (!md_->cache_voxel_.empty()) {
    ...
    const bool was_unknown = md_->occupancy_buffer_[adr] < mp_->clamp_min_log_ - 1e-3;
    const bool was_occupied = md_->occupancy_buffer_[adr] > mp_->min_occupancy_log_;
    if (was_unknown) {
    ...
    md_->occupancy_buffer_[adr] =
        std::min(std::max(md_->occupancy_buffer_[adr] + log_odds_update, mp_->clamp_min_log_),
            mp_->clamp_max_log_);
    const bool is_occupied = md_->occupancy_buffer_[adr] > mp_->min_occupancy_log_;
    // Send both first observations and later FREE/OCCUPIED state transitions.
    // Reusing an address in a newer chunk gives receivers the latest static-map state.
    if (was_unknown || was_occupied != is_occupied) new_voxel_ids_.push_back(adr);
  }
...
void SDFMap::clearAndInflateLocalMap() {
  ...
  // Inflate from a halo around the cleared box, otherwise an occupied voxel
  // just outside the box would no longer contribute to cells inside it.
  Eigen::Vector3i scan_min = md_->local_bound_min_ - Eigen::Vector3i::Constant(inf_step);
  Eigen::Vector3i scan_max = md_->local_bound_max_ + Eigen::Vector3i::Constant(inf_step);
  boundIndex(scan_min);
  boundIndex(scan_max);
