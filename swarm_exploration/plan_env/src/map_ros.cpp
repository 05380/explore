...
void MapROS::init() {
  ...
  depth_filter_margin_ = std::max(0, depth_filter_margin_);
  k_depth_scaling_factor_ = std::max(1.0, k_depth_scaling_factor_);
...
void MapROS::processDepthImage() {
  ...
  const int usable_cols = std::max(0, cols - 2 * depth_filter_margin_);
  const int usable_rows = std::max(0, rows - 2 * depth_filter_margin_);
  const size_t max_points =
      static_cast<size_t>((usable_cols + skip_pixel_ - 1) / skip_pixel_) *
      static_cast<size_t>((usable_rows + skip_pixel_ - 1) / skip_pixel_);
  if (point_cloud_.points.size() < max_points) point_cloud_.points.resize(max_points);
  if (proj_points_.size() < max_points) proj_points_.resize(max_points);
  double depth;
  ...
  for (int v = depth_filter_margin_; v < rows - depth_filter_margin_; v += skip_pixel_) {
    ...
    for (int u = depth_filter_margin_; u < cols - depth_filter_margin_; u += skip_pixel_) {
      ...
      // Invalid/no-return pixels must not become a ring of occupied voxels at
      // max range. Keep only actual returns inside the calibrated usable range.
      if (raw_depth == 0 || !std::isfinite(depth) || depth > depth_filter_maxdist_ ||
          depth < depth_filter_mindist_)
        continue;
