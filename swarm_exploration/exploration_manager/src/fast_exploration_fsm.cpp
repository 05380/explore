...
void FastExplorationFSM::FSMCallback(const ros::TimerEvent& e) {
  ...
  switch (state_) {
    ...
    case PLAN_TRAJ: {
      if (fp_->use_rl_navigation_ || fd_->static_state_) {
      ...
      if (!fp_->use_rl_navigation_) replan_pub_.publish(std_msgs::Empty());
    ...
    case EXEC_TRAJ: {
      ...
      if (fp_->use_rl_navigation_) {
        ...
        if (fd_->go_back_ && target_dist < fp_->rl_target_reached_dist_) {
          clearVisMarker();
          transitState(FINISH, "FSM-RL");
          return;
        }
...
void FastExplorationFSM::safetyCallback(const ros::TimerEvent& e) {
  if (fp_->use_rl_navigation_) return;
